...
# Extract the data and save in subfolder win_cpu/ as both .json and .csv files.
#
# Objective:
...
def main():
    ...
    for index in range(len(link_list)):
        ...
        filename, folder = create_filename(link, title_str)
        ...
        #print("Sub-folder and File name: {}{}".format(folder, full_filename))
        ...
        dump_to_json(data_dict, folder, full_filename)
        dump_to_csv(data_dict, folder, full_filename)

    print("\nCompleted. json and csv files in subfolder: {}".format(sub_folder))
...
def create_filename(link, title_str):
    ...
    # Create a sub-folder for the json and csv files. E.g. "win_cpu"
    #print("If required, creating the sub-folder to store json files: {}.".format(folder))
    folder = sub_folder
...
def dump_to_json(data_dict, folder, filename):
    # Dump the data_dict as a .json file in the sub-folder
    with open(folder + filename + ".json", 'w') as fout:
        fout.write(json.dumps(data_dict, indent = 4))


def dump_to_csv(data_dict, folder, filename):
    ...
    # Use csv to write list to sub-folder csv file
...
Completed. json and csv files in subfolder: win_cpu
